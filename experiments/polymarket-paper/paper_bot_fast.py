...
class Feed:
    ...
    def discover(self, limit=12):
        ...
        raw = get_json(GAMMA+"/markets", {"active":"true","closed":"false","limit":limit,"order":"volume_24hr","ascending":"false"})
        out=[]
        for m in raw:
...
class Bot:
    def __init__(self, cfg, out, offline=False, seed=7):
        ...
        self.current_market_id=None; self.hold_counter=0
    ...
    def tick(self, market):
        if market["id"] != self.current_market_id and not self.pos:
            self.current_market_id=market["id"]; self.mids=[]; self.hold_counter=0
        ...
        exploration=(self.hold_counter >= int(self.cfg["exploration_after_holds"]))
        signal=(mom>float(self.cfg["entry_momentum"]) and expected_net>float(self.cfg["min_expected_profit_usd"]))
        if regime=="normal" and liquidity>=float(self.cfg["min_book_liquidity_shares"]) and (signal or exploration):
            if exploration: size=min(size,float(self.cfg["exploration_position_usd"]))
            shares=size/ask
            fee=self.fee(shares,ask); cost=shares*ask+fee
            if cost<=self.cash:
                self.cash-=cost; self.pos=Position(market["id"],token,market["question"],shares,ask,time.time(),fee)
                self.log_reason("OPEN","exploration_activity_test" if exploration else "positive_net_momentum",expected_net=expected_net)
                self.hold_counter=0
        else:
            self.hold_counter+=1
            self.log_reason("HOLD","entry_filters",regime=regime,momentum=mom,expected_net=expected_net,liquidity=liquidity)
    ...
    def run(self, seconds, interval):
        ...
        target=float(seconds); last_save=time.time(); markets=[]; market_index=0; last_rotation=time.time()
        try:
            while self.accumulated_seconds+(time.time()-segment_started) < target:
                try:
                    ...
                    if not self.pos and time.time()-last_rotation >= float(self.cfg["market_rotate_seconds"]):
                        market_index=(market_index+1)%len(markets); last_rotation=time.time()
                    self.tick(markets[market_index])
...
DEFAULT={"initial_balance_usd":100,"max_position_usd":5,"max_total_drawdown_pct":.05,"max_spread":.04,"volatility_regime_threshold":.025,"min_book_liquidity_shares":50,"entry_momentum":.0005,"momentum_horizon_multiplier":5,"min_expected_profit_usd":.001,"take_profit_usd":.02,"position_stop_usd":.05,"max_hold_seconds":30,"taker_fee_rate":.05,"market_limit":30,"exploration_after_holds":10,"exploration_position_usd":2,"market_rotate_seconds":45}
def main():
